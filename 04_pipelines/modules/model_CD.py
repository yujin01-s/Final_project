...
from modules.feature_selector import generate_cd_derived_features, cd_final_features
...
def run_cd_modeling(df, used_cols, save_path=None):
    print("🔍 CD 세그먼트 모델링 시작")

    # 1. 파생변수 생성
    df = generate_cd_derived_features(df)

    # 2. CD 세그먼트만 선택
    df = df[df["Segment"].isin([2, 3])]
    X = df[cd_final_features]
    y = df["Segment"]
    ...
    train_and_evaluate(model, X_train, y_train, X_val, y_val, model_name="XGB_CD")
    ...
    # 8. 저장
    if save_path:
        save_model_and_results(model, model_name="XGB_CD", save_path=save_path)

    return {
        "df": df,
        "features": cd_final_features,
        "X_train": X_train,
        "X_val": X_val,
        "y_train": y_train,
        "y_val": y_val,
